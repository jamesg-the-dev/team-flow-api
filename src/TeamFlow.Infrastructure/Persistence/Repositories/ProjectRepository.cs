...
internal sealed class ProjectRepository : IProjectRepository
{
    ...
    public ProjectRepository(TeamFlowDbContext ctx, IUnitOfWork unitOfWork) { _ctx = ctx; UnitOfWork = unitOfWork; }
    ...
    public Task<Project?> GetByIdWithMembersAsync(Guid id, CancellationToken ct = default) =>
        _ctx.Projects.Include(p => p.Members).Include(p => p.Tags)
            .FirstOrDefaultAsync(p => p.Id == id, ct);

    public Task<bool> KeyExistsAsync(Guid workspaceId, string key, CancellationToken ct = default) =>
        _ctx.Projects.IgnoreQueryFilters()
