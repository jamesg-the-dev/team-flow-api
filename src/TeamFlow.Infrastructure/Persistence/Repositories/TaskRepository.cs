...
internal sealed class TaskRepository : ITaskRepository
{
    ...
    public TaskRepository(TeamFlowDbContext ctx, IUnitOfWork unitOfWork)
    {
        _ctx = ctx;
    ...
    public async Task<IReadOnlyList<TaskItem>> ListByColumnAsync(
        Guid projectId,
        TaskColumn column,
        CancellationToken ct = default
    ) =>
        await _ctx
            .Tasks.Where(t => t.ProjectId == projectId && t.Column == column)
            .OrderBy(t => t.Position)
            .ToListAsync(ct);

    public async Task<(decimal? Min, decimal? Max)> GetColumnPositionBoundsAsync(
        Guid projectId,
        TaskColumn column,
        CancellationToken ct = default
    )
    {
        var bounds = await _ctx
            .Tasks.Where(t => t.ProjectId == projectId && t.Column == column)
            .GroupBy(_ => 1)
            .Select(g => new
            {
                Min = (decimal?)g.Min(t => t.Position),
                Max = (decimal?)g.Max(t => t.Position),
            })
            .FirstOrDefaultAsync(ct);
        return (bounds?.Min, bounds?.Max);
    }

    public async Task<decimal?> GetNeighbourPositionAsync(
        Guid taskId,
        bool before,
        CancellationToken ct = default
    )
    {
        var pivot = await _ctx
            .Tasks.AsNoTracking()
            .Where(t => t.Id == taskId)
            .Select(t => new
            {
        ...
        var sibling = before
            ? await _ctx
                .Tasks.AsNoTracking()
                .Where(t =>
                    t.ProjectId == pivot.ProjectId
                    && t.Column == pivot.Column
