...
public sealed record CreateTaskCommand(
    ...
    DateOnly? DueDate) : ICommand<TaskDto>;
...
internal sealed class CreateTaskHandler : ICommandHandler<CreateTaskCommand, TaskDto>
{
    ...
    public CreateTaskHandler(IProjectRepository projects, ITaskRepository tasks, ICurrentUser currentUser)
    {
        _projects = projects; _tasks = tasks; _currentUser = currentUser;
    }
    ...
    public async Task<Result<TaskDto>> Handle(CreateTaskCommand request, CancellationToken ct)
    {
        ...
        var (_, max) = await _tasks.GetColumnPositionBoundsAsync(project.Id, TaskColumn.Backlog, ct);
        ...
        return new TaskDto(task.Id, task.ProjectId, task.Number, task.Title, task.Description,
            task.Column, task.Priority, task.Position, task.AssigneeId, task.ReporterId,
            task.EstimateHours, task.DueDate, task.CompletedAt, task.CreatedAt, task.UpdatedAt);
    }
}
