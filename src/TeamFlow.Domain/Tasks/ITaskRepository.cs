...
public interface ITaskRepository : IRepository<TaskItem>
{
    ...
    Task<IReadOnlyList<TaskItem>> ListByColumnAsync(Guid projectId, TaskColumn column, CancellationToken ct = default);

    /// <summary>Returns the min / max Position in a column (used to compute fractional insert positions).</summary>
    Task<(decimal? Min, decimal? Max)> GetColumnPositionBoundsAsync(Guid projectId, TaskColumn column, CancellationToken ct = default);

    /// <summary>Returns the position immediately before/after a target task in the same column.</summary>
    Task<decimal?> GetNeighbourPositionAsync(Guid taskId, bool before, CancellationToken ct = default);
