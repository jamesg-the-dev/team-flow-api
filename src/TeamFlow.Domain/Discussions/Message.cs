...
public sealed class Message : AggregateRoot, ISoftDeletable
{
    ...
    public static Message Post(Guid channelId, Guid authorId, string body, Guid? parentId = null,
        IEnumerable<Guid>? mentions = null)
    {
        if (string.IsNullOrWhiteSpace(body)) throw DomainException.Invariant("Message body is required.");
        if (body.Length > 10_000) throw DomainException.Invariant("Message body too long.");
        ...
        if (mentions is not null)
            foreach (var u in mentions.Distinct()) msg._mentions.Add(new MessageMention(msg.Id, u));
    ...
    {
        if (editorId != AuthorId) throw DomainException.Invariant("Only the author can edit a message.");
    ...
    {
        if (string.IsNullOrWhiteSpace(emoji)) throw DomainException.Invariant("Emoji required.");
...
public sealed class MessageReaction
{
    ...
    { MessageId = messageId; UserId = userId; Emoji = emoji; CreatedAt = DateTimeOffset.UtcNow; }
}
...
public sealed class MessageMention
{
    ...
    internal MessageMention(Guid messageId, Guid userId) { MessageId = messageId; UserId = userId; }
}

public interface IMessageRepository : IRepository<Message>
{
    Task<Message?> GetByIdAsync(Guid id, CancellationToken ct = default);
    Task<IReadOnlyList<Message>> GetChannelTimelineAsync(Guid channelId, DateTimeOffset? before, int take, CancellationToken ct = default);
